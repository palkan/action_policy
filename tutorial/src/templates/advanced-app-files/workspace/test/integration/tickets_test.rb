...
class TicketsTest < ActionDispatch::IntegrationTest
  ...
  test "resolve changes ticket status for agent with sufficient level" do
    sign_in users(:bob)
    patch resolve_ticket_path(@ticket) # password_reset is level 1, Bob is level 2
  ...
  test "resolve denies agent with insufficient level" do
    sign_in users(:bob)
    patch resolve_ticket_path(tickets(:billing)) # billing is level 3, Bob is level 2
    assert_redirected_to tickets_path
    follow_redirect!
    assert_text "Your level is insufficient to resolve this ticket"
  end

  test "resolve denies customer" do
    sign_in @alice
    patch resolve_ticket_path(@ticket)
    assert_redirected_to tickets_path
    follow_redirect!
    assert_text "Customers are not allowed to resolve tickets"
  end
end
