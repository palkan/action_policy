...
class CommentPolicyTest < ActiveSupport::TestCase
  ...
  test "relation_scope hides internal comments from customer" do
    scope = CommentPolicy.new(Comment, user: users(:alice)).apply_scope(Comment.all, type: :active_record_relation)
    assert scope.none?(&:internal?)
  end

  test "relation_scope shows all comments to agent" do
    scope = CommentPolicy.new(Comment, user: users(:bob)).apply_scope(Comment.all, type: :active_record_relation)
